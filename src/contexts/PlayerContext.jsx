/**
 * Player Context
 * Manages YouTube player state and controls (Consolidated to fix initialization issues)
 */
import {
  createContext,
  useContext,
...
export function PlayerProvider({ children }) {
  ...
  // Define actions as refs first to avoid circular dependency in hooks
  const loadTrackRef = useRef(null);
  const seekRef = useRef(null);

  // 1. Queue logic - Define this FIRST as other hooks might need it
  const queueManager = usePlayerQueue(
    (track, pos) => loadTrackRef.current?.(track, pos),
    state.currentTime,
    (time) => seekRef.current?.(time),
    playVideo,
    (playing) => dispatch({ type: ACTIONS.SET_PLAYING, payload: playing }),
  );

  // 2. Core Player logic (Lifecycle, Events, Intervals)
  const {
    playerRef,
    containerId,
    handleStateChange,
...
export function usePlayer() {
  ...
  if (state === undefined || actions === undefined) {
    // This could happen if the hook is called outside Provider
    // during a weird initialization phase
    return {};
  }
  return {
    ...state,
